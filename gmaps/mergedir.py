...
def merge_tiles(x1, y1, x2, y2, z):
    ...
    for y in range(leny):
        for x in range(lenx):
            small_pic = pil.open("/data/temp/pietro/"+str(pos1y+y)+"_"+str(pos1x+x)+"_tile.jpeg")
...
if __name__ == '__main__':
    ...
    for i in range(items-1):
        for j in range(items-1):
            ...
            outpic = merge_tiles(left, top, right, bottom, zoom)
            ...
            saveTiff(r, g, b, gt, "/data/temp/"+str(i)+"_"+str(j)+"_campania.tif")
            sys.exit(0)